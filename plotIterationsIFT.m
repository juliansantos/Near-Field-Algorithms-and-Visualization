function plotIterationsIFT(i, x_mesh, y_mesh, M_layer, ME_layer)
    ...
    if i==1 || ~isinteger( i ) 
        ...
        subplot(2,4,5); surf(x_mesh,y_mesh, angle(M_layer(:,:,1)));  title('\angle Layer 1-- Sim. Values');   colorbar ; shading interp; view(0,90);
        subplot(2,4,2); surf(x_mesh,y_mesh, mag2db(abs(M_layer(:,:,2))));  title('|Layer 2| -- Sim. Values');   colorbar ; shading interp; view(0,90);
        subplot(2,4,6); surf(x_mesh,y_mesh, angle(M_layer(:,:,2))-angle(M_layer(52,52,2)));  title('\angle Layer 2 -- Sim. Values');   colorbar ; shading interp;  view(0,90);
    end
    ...
    subplot(2,4,7); cla; surf(x_mesh,y_mesh, angle((ME_layer(:,:,1))));  title(['\angle Layer 1 -- Estimated Values iter:' num2str(i)]);   colorbar ; shading interp;  view(0,90);
    ...
    subplot(2,4,8); cla; surf(x_mesh,y_mesh, angle((ME_layer(:,:,2))));  title(['\angle Layer 2 -- Estimated Values iter:' num2str(i)]);  colorbar ; shading interp;  view(0,90);   
    pause(1e-3)
